...
- **Apply the ROI filter before writing each test.** From `docs/rules/tdd.md` § "Test ROI": ask "if I deleted this test, could a real bug slip through that the compiler, type system, or code review would miss?" If the answer is no — the test only mirrors structure (constant value, field name string literal, trivial getter) — skip it and note the omission in your handoff. Do not write a test just because the contract listed it; the contract may be wrong.
